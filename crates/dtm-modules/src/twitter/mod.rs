...
impl TwitterModule {
    fn load_checks(&self) -> Vec<PrivacyCheck> {
        checklist::load_checks("twitter").unwrap_or_default()
    }
}

#[async_trait]
impl Module for TwitterModule {
    fn name(&self) -> &str {
        "twitter"
    }
    ...
    fn description(&self) -> &str {
        "Interactive privacy checklist for Twitter/X settings"
    }
        ...
        if checks.is_empty() {
            return Ok(AuditResult {
                ...
                findings: vec![Finding {
                    ...
                    description:
                        "Could not load Twitter privacy checks from shared/checklists/twitter.yaml"
                            .to_string(),
...
mod tests {
    ...
    fn module_display_name() {
        assert_eq!(module().display_name(), "Twitter/X Privacy");
    }
    ...
    fn checklist_loads() {
        ...
        assert!(checks.is_some(), "Twitter checklist() should return Some");
    }
    ...
    fn checklist_has_checks() {
        ...
        assert!(
            checks.len() >= 13,
            "Twitter checklist should have at least 13 checks, got {}",
            checks.len()
        );
    }
    ...
    async fn audit_non_interactive_returns_result() {
        ...
        assert_eq!(result.module_name, "twitter");
