...
pub struct TikTokModule;

impl TikTokModule {
    fn load_checks(&self) -> Vec<PrivacyCheck> {
        checklist::load_checks("tiktok").unwrap_or_default()
    }
}

#[async_trait]
impl Module for TikTokModule {
    fn name(&self) -> &str {
        "tiktok"
    }
    ...
    fn description(&self) -> &str {
        "Interactive privacy checklist for TikTok settings"
    }
        ...
        if checks.is_empty() {
            return Ok(AuditResult {
                ...
                findings: vec![Finding {
                    ...
                    description:
                        "Could not load TikTok privacy checks from shared/checklists/tiktok.yaml"
                            .to_string(),
...
mod tests {
    ...
    fn module_display_name() {
        assert_eq!(module().display_name(), "TikTok Privacy");
    }
    ...
    fn checklist_loads() {
        ...
        assert!(checks.is_some(), "TikTok checklist() should return Some");
    }
    ...
    fn checklist_has_checks() {
        ...
        assert!(
            checks.len() >= 10,
            "TikTok checklist should have at least 10 checks, got {}",
            checks.len()
        );
    }
    ...
    async fn audit_non_interactive_returns_result() {
        ...
        assert_eq!(result.module_name, "tiktok");
