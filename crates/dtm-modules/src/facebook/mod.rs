...
pub struct FacebookModule;

impl FacebookModule {
    fn load_checks(&self) -> Vec<PrivacyCheck> {
        checklist::load_checks("facebook").unwrap_or_default()
    }
}

#[async_trait]
impl Module for FacebookModule {
    fn name(&self) -> &str {
        "facebook"
    }
    ...
    fn description(&self) -> &str {
        "Interactive privacy checklist for Facebook settings"
    }
        ...
        if checks.is_empty() {
            return Ok(AuditResult {
                ...
                findings: vec![Finding {
                    ...
                    description: "Could not load Facebook privacy checks from shared/checklists/facebook.yaml".to_string(),
...
mod tests {
    ...
    #[test]
    fn module_name() {
        assert_eq!(module().name(), "facebook");
    }
    ...
    fn module_display_name() {
        assert_eq!(module().display_name(), "Facebook Privacy");
    }
    ...
    fn checklist_loads() {
        ...
        assert!(checks.is_some(), "Facebook checklist() should return Some");
    }
    ...
    fn checklist_has_checks() {
        ...
        assert!(
            checks.len() >= 14,
            "Facebook checklist should have at least 14 checks, got {}",
            checks.len()
        );
    }
    ...
    async fn audit_non_interactive_returns_result() {
        ...
        assert_eq!(result.module_name, "facebook");
